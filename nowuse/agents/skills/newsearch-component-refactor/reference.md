...
## SLS 可选附录（仅 sls_logs_list 场景启用）

当页面接口是 `sls_logs_list`（或等价 SLS Query/SQL）时，在基线 `getList` 上追加下述拼装。
...
let fromTime = null
let toTime = null
let searchCond = (data && data.search_cond) || {}

Object.keys(searchCond).forEach(key => {
    let value = searchCond[key]
    if (key == "created_at__range") {
        ...
        whereText = value || ""
    } else if (key.indexOf("__in") > -1 && Array.isArray(value) && value.length) {
        let pureKey = key.replace("__in", "")
        queryList.push(`(${value.map(item => `${pureKey} : '${item}'`).join(" or ")})`)
...
let listReq = processInKeys({
    logstore: data.logstore,
    from_time: fromTime,
    to_time: toTime,
    query: `${fullText} | SELECT ... FROM log ${whereText} order by ctime desc limit ${offset},${data.limit}`
})
let totalReq = processInKeys({
    logstore: data.logstore,
    from_time: fromTime,
    to_time: toTime,
    query: `${fullText} | SELECT count(*) total FROM log ${whereText}`
})
```

### 3）SLS 场景提示

- `text` 只做全文追加，不当作 `key : value`
- `where` 只做 SQL 片段，不参与 `queryList.join("and")`
- `created_at__range` 建议固定保留键位（值可空）
- 是否启用总数查询按页面需求决定；若禁用，页面需有 `total` 兜底