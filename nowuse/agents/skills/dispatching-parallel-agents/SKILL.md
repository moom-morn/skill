...
description: 当面临 2 个以上可独立完成且无需共享状态或顺序依赖的任务时使用
---
...
你将任务委托给具有隔离上下文的专门代理。通过精确构造它们的指令和上下文，确保它们保持专注并成功完成任务。它们永远不应继承你会话的上下文或历史记录——你精确构造它们需要的内容。这也为你自己的协调工作保留了上下文。

当你有多个不相关的失败（不同测试文件、不同子系统、不同 bug）时，按顺序调查浪费时间。每个调查是独立的，可以并行进行。
